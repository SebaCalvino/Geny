# -*- coding: utf-8 -*-
"""
Factory de mini-webs — OVERHAUL
NO cambia tu esquema de output: la ruta final la define el orquestador.
Cumple:
- Paletas rotativas (4) con variables CSS
- Fondo con gradiente/patrones sutiles (AA)
- 4 páginas: index / about / services / contact
- 4+ imágenes (placeholders remotos) + brandmark SVG inline
- Historia + datos ficticios creíbles (dirección, teléfono, email, redes, horarios)
...
def pick_palette(index_hint: int | None, seed: int) -> tuple[dict, int]:
    if index_hint is not None:
        ...
        return PALETTES[i], i
    random.seed(seed)
    i = random.randint(0, len(PALETTES)-1)
    return PALETTES[i], i
...
def tpl_styles(palette: dict) -> str:
    return dedent(f"""\
    /* styles.css — variables + gradientes accesibles */
    :root {{
      --bg:{palette['bg']};
      --fg:{palette['fg']};
      --acc1:{palette['acc1']};
      --acc2:{palette['acc2']};
      --muted:#9aa0a6;
      --card:color-mix(in oklab, var(--bg), white 6%);
      --border:color-mix(in oklab, var(--bg), white 14%);
      --radius:14px;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .btn-primary {{ background: var(--acc1); color: white; border-color: color-mix(in oklab, var(--acc1), black 10%); }}
...
def generate_website(output_dir: str, site_title: str, palette_index: int | None = None) -> dict:
    """
    NO cambia el path final: usa 'output_dir' tal como venga del orquestador.
    Devuelve info básica para logs/commits.
    """
    brand = site_title
    ...
    base = Path(output_dir)  # Respeta tu ruta
    ...
    # Páginas (con marcado de nav activo)
    ...
    about_html = page_wrap(
        f"{brand} — Sobre nosotros",
        "",
        header.replace('>Nosotros<', ' class="active">Nosotros<'),
        tpl_about(company),
        footer
    )
    services_html = page_wrap(
        f"{brand} — Servicios",
        "",
        header.replace('>Servicios<', ' class="active">Servicios<'),
        tpl_services(),
        footer
    )
    contact_html = page_wrap(
        f"{brand} — Contacto",
        "",
        header.replace('>Contacto<', ' class="active">Contacto<'),
        tpl_contact(company),
        footer
    )
    ...
    metadata = {
        ...
        "images": [
            f"assets/images/hero_{slug}.jpg",
            "assets/images/section_1.jpg",
            "assets/images/section_2.jpg",
            "assets/images/brandmark.svg"
        ],
        "created_at": now_iso()
    }
...
# Uso manual (ejemplo):
# info = generate_website(output_dir="output/2025-09-16/nebulacare", site_title="NébulaCare", palette_index=None)
# print(info)